...
class DataCollector:
    ...
    def post_process(self, raw_frame_detections: List[Dict]) -> List[RallyData]:
        """
        ...
        """
        for frame in raw_frame_detections:
            self.collect_frame_data(**frame)

        processed_rallies = self._segment_rallies()
        for i in range(len(processed_rallies)):
            processed = self._post_process_rally(processed_rallies[i].rally_frames)
            processed_rallies[i].rally_frames = processed
