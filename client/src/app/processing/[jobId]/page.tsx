  ...
  useEffect(() => {
    if (statusData?.status === JobStatus.COMPLETED && jobData?.video_id) {
      ...
      setTimeout(() => {
        ...
        router.replace(`/dashboard/${jobData.video_id}/overview`);
      }, 1500);
    }
        ...
        <div className="w-full max-w-2xl space-y-12">
          ...
          <div className="text-center space-y-4">
            <h2 className="text-3xl font-semibold text-gray-900">
              {status === JobStatus.PENDING && 'Preparing Analysis...'}
              {status === JobStatus.PROCESSING && 'Analyzing Match...'}
              {status === JobStatus.COMPLETED && 'Analysis Complete!'}
              {status === JobStatus.FAILED && 'Analysis Failed'}
              {status === JobStatus.CANCELLED && 'Analysis Cancelled'}
            </h2>

            {currentStage && status === JobStatus.PROCESSING && (
              <p className="text-lg text-gray-600">{currentStage}</p>
            )}
