...
export function Sidebar({
    ...
    <div className="h-full flex flex-col bg-card">
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-border/20 shrink-0">
        ...
        <Button
          variant="ghost"
          size="icon"
          onClick={() => onMobileToggle?.() ?? setInternalMobileOpen(false)}
          className="md:hidden"
      ...
      <nav className="p-2 flex flex-col flex-1 overflow-hidden">
        ...
        <div className="mt-auto pt-2">
          ...
          >
            ...
            {!collapsed && <span className="ml-2 font-medium">New Analysis</span>}
          </Button>
        </div>
      </nav>
    </div>
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "hidden md:block h-screen bg-card shadow-lg border-r border-border/20 transition-all duration-300",
          collapsed ? "w-20" : "w-64"
        )}
      >
        <SidebarContent />
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 h-screen w-64 bg-card shadow-2xl border-r border-border/20 z-50 transition-transform duration-300 md:hidden",
