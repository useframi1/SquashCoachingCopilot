...
class VideoHandler:
    ...
    def __init__(self, input_path: str, output_path: str, codec: str = "avc1"):
        """
    ...
    def write_video(
        ...
        try:
            with VideoWriter(
                output_path or self.output_path, self.metadata, self.codec
            ) as writer:
                frame_count = 0
                for frame in frames:
                    writer.write(frame)
                ...
                print(f"Successfully wrote {frame_count} frames to {self.output_path}")

        except PermissionError as e:
            raise IOError(f"Permission denied writing to {self.output_path}: {str(e)}")
        except OSError as e:
            raise IOError(f"Error writing video (disk space?): {str(e)}")
        except Exception as e:
            raise ValueError(f"Error writing video {self.output_path}: {str(e)}")
