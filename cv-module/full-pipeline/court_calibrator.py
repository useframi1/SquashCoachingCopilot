...
class CourtCalibrator:
    def __init__(self):
        ...
        self.model = YOLO(self.config["court_calibrator"]["model_path"], verbose=False)
    ...
    def _get_real_coords(self):
        return np.array(
            self.config["court_calibrator"]["real_coords"],
            dtype=np.float32,
        )
    ...
    def detect_keypoints(self, frame):
        ...
        for person_keypoints in keypoints_array:
            for idx, (x, y, conf) in enumerate(person_keypoints):
                if conf > self.config["court_calibrator"]["confidence_threshold"]:
