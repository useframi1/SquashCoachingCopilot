...
class PlayerTracker:
    def __init__(
        ...
        self.model = YOLO(self.config["player_tracker"]["model_path"], verbose=False)
        self.player_class_id = self.config["player_tracker"]["player_class_id"]
            ...
            if int(cls) == self.player_class_id:
                ...
                detections.append(
                    (
                        [x, y, w, h],
                        conf,
                        self.config["player_tracker"]["player_class_name"],
                    )
                )
        return detections
