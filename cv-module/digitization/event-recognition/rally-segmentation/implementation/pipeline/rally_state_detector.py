...
class RallyStateDetector:
    ...
    def __init__(self, config):
        ...
        self.rally_active_avg_distance_threshold = self.rally_config.get(
            "rally_active_avg_distance_threshold", 3.5
        )
        self.rally_end_avg_distance_threshold = self.rally_config.get(
            "rally_end_avg_distance_threshold", 5.5
        )
    ...
    def is_rally_active(self, p1_pos, p2_pos, combined_intensity, avg_distance):
        """Check if conditions indicate active rally"""
        if (
            p1_pos is None
            or p2_pos is None
            or combined_intensity is None
            or avg_distance is None
        ):
            return False

        # Average distance should be low for active rally
        if avg_distance >= self.rally_active_avg_distance_threshold:
            return False

        # Players should not be in start positions
        return not self.is_rally_start(p1_pos, p2_pos, 0.0)

    def is_rally_end(self, p1_pos, p2_pos, combined_intensity, avg_distance):
        ...
        # Average distance should be high for rally end
        if avg_distance < self.rally_end_avg_distance_threshold:
            return False

        # Players should be on different sides
        return self.is_players_on_different_sides(p1_pos, p2_pos)
    ...
    def update_state(
        ...
        if self.current_state == "rally_end":
            ...
            if self.is_rally_active(p1_pos, p2_pos, combined_intensity, avg_distance):
                new_state = "rally_active"
                state_changed = True

        elif self.current_state == "rally_active":
            if self.is_rally_end(p1_pos, p2_pos, combined_intensity, avg_distance):
                new_state = "rally_end"
                state_changed = True
