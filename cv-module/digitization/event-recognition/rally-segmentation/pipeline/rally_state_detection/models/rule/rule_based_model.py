...
class RuleBasedModel:
    ...
    def _is_behind_service_line(self, player_y: float) -> bool:
        ...
        return player_y > self.config["models"]["rule_based"]["service_line_y"]

    def _is_in_service_box(self, player_x: float, player_y: float, side: str) -> bool:
        """Check if a player is in their service box."""
        box = self.config["models"]["rule_based"]["service_boxes"][side]
        return (
    ...
    def _are_on_opposite_sides(self, p1_x: float, p2_x: float) -> bool:
        ...
        court_center_x = self.config["models"]["rule_based"]["court_center_x"]
    ...
    def _get_distance_based_state(self, distance: float) -> str:
        ...
        if distance <= self.config["models"]["rule_based"]["active_state"]["distance_max"]:
            return "active"
        elif (
            self.config["models"]["rule_based"]["start_state"]["distance_min"]
            <= distance
            <= self.config["models"]["rule_based"]["start_state"]["distance_max"]
        ):
            return "start"
        elif distance >= self.config["models"]["rule_based"]["end_state"]["distance_min"]:
            return "end"
        else:
    ...
    def predict(self, base_metrics: dict) -> str:
        ...
        Args:
            base_metrics: Dictionary containing distance and position metrics

        Returns:
            Predicted state: 'start', 'active', or 'end'
        """
        distance = base_metrics["mean_distance"]

        # Check position constraints for start state
        start_position_ok = self._check_start_position_constraints(base_metrics)
        start_distance_ok = (
            self.config["models"]["rule_based"]["start_state"]["distance_min"]
            <= distance
            <= self.config["models"]["rule_based"]["start_state"]["distance_max"]
        ...
        if distance >= self.config["models"]["rule_based"]["end_state"]["distance_min"]:
        ...
        if self.start_condition_frames >= self.config["models"]["rule_based"]["lookback_frames"]:
            suggested_state = "start"

        # Rule 2: Go to active state when players are in close range
        elif distance <= self.config["models"]["rule_based"]["active_state"]["distance_max"]:
            suggested_state = "active"

        # Rule 3: Go to end state when distance stays above threshold for required frames
        elif self.end_condition_frames >= self.config["models"]["rule_based"]["lookback_frames"]:
            suggested_state = "end"
        else:
    ...
    def predict_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in df.iterrows():
            base_metrics = {
                ...
                "median_player1_x": row["median_player1_x"],
                "median_player1_y": row["median_player1_y"],
                "median_player2_x": row["median_player2_x"],
                "median_player2_y": row["median_player2_y"],
            }
            prediction = self.predict(base_metrics)
