...
from stroke_detection_pipeline.model.lstm_classifier import LSTMClassifier
from stroke_detection_pipeline.utils import (
    load_config,
    get_package_dir,
    extract_keypoints,
    normalize_keypoints,
...
class StrokeDetector:
    ...
    def __init__(self, config: dict = None):
        ...
        self.model_path = os.path.join(
            get_package_dir(), self.config["model"]["model_path"]
        )
    ...
    def process_frame(
        self, player_keypoints: Dict[int, any]
        ...
        Args:
            player_keypoints: Dictionary mapping player_id to their keypoints
                ...
                where keypoints_data can be:
                  ...
                  - Tensor/Array: COCO format keypoints (17 keypoints)

        Returns:
            Dictionary mapping player_id to prediction:
                {player_id: {'stroke': str, 'confidence': float}, ...}
        """
        predictions = {}
        ...
        for player_id, keypoints_data in player_keypoints.items():
            try:
                ...
                if len(self.player_buffers[player_id]) >= self.window_size:
                    ...
                    # Check cooldown
                    if self._check_cooldown(player_id, stroke_type):
