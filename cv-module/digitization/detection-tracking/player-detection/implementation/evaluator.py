...
class SquashTrackerEvaluator:
    def __init__(self):
        ...
        # Per-player metrics - using category IDs from config
                ...
                ground_truth[frame_name].append(
                    {
                        ...
                        "class_id": ann[
                            "category_id"
                        ],  # Keep original category_id (1 or 2)
    ...
    def establish_id_mapping(self, predictions, ground_truth_frame, frame_name):
        ...
        if iou_matrix.size > 0:
            ...
            if self.id_mapping is None or current_confidence > self.mapping_confidence:
                ...
                print(
                    f"Updated ID mapping at {frame_name}: {self.id_mapping} (confidence: {current_confidence:.3f})"
                )
        ...
        self.frame_results.append(
            {
                ...
                "true_positives": (
                    len(matched_pred) if "matched_pred" in locals() else 0
                ),
