...
from player_tracking.utils import load_config, get_package_dir
...
class PlayerTracker:
    ...
    def postprocess(self, positions_history):
        """
        Apply interpolation for missing positions and smoothing filter.

        Args:
            positions_history: Dictionary with player IDs as keys and lists of positions as values.
                              Each position can be a tuple (x, y) or None for missing frames.
                              Example: {1: [(x1, y1), None, (x3, y3), ...], 2: [...]}

        Returns:
            dict: Dictionary with player IDs as keys and lists of smoothed positions as values
        """
        smoothed_positions = {}

        for player_id, positions in positions_history.items():
            ...
            if len(valid_indices) > 1:
                ...
                interp_x = interp1d(valid_indices, valid_x, kind='linear',
                                   fill_value='extrapolate', bounds_error=False)
                interp_y = interp1d(valid_indices, valid_y, kind='linear',
                                   fill_value='extrapolate', bounds_error=False)
            ...
            smoothed_positions[player_id] = [(x, y) for x, y in zip(smoothed_x, smoothed_y)]
