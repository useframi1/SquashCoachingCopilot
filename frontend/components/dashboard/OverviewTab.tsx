...
export default function OverviewTab({results}: OverviewTabProps) {
    ...
    const rallyDurationData = results.rallies.map((rally, idx) => ({
        name: `${idx + 1}`,
        duration: parseFloat(
            ((rally.end_frame - rally.start_frame) / results.fps).toFixed(1)
        ),
    ...
    // Count total shots (ball wall hits) across all rallies
    let totalShots = 0;

    results.rallies.forEach((rally) => {
        rally.rally_frames.forEach((frame) => {
            if (frame.ball.is_wall_hit) {
                totalShots++;
            }
        });
    });
    ...
    const wallHitData = results.rallies.map((rally, idx) => {
        const wallHits = rally.rally_frames.filter(
            (frame) => frame.ball.is_wall_hit
        ).length;
        return {
            name: `${idx + 1}`,
            hits: wallHits,
    ...
    const rallyLengthCategories = {
        ...
        medium: rallyDurationData.filter(
            (r) => r.duration >= 5 && r.duration < 15
        ).length,
        long: rallyDurationData.filter(
            (r) => r.duration >= 15 && r.duration < 30
        ).length,
