 ...
 */
export function getCourtZone(position: [number, number]): 'front' | 'mid' | 'back' {
  const [, y] = position;
...
export function calculateTDominance(
  ...
  const nearTCount = positions.filter(pos => distanceFromT(pos) <= threshold).length;
 ...
 */
export function calculateAveragePosition(positions: [number, number][]): [number, number] {
 ...
 */
export function calculateMovementIntensity(positions: [number, number][]): number {
 ...
 */
export function getAggressivenessTimeline(positions: [number, number][]): number[] {
