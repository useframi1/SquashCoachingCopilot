...
from court_calibration.utils import load_config
...
class CourtCalibrator:
    ...
    def process_frame(self, frame):
        ...
        Returns:
            homographies: Dict with keys 'floor' and 'wall' mapping to homography matrices
            keypoints_per_class: Dict mapping class_name to detected keypoints
        """
        keypoints_per_class = self.detect_keypoints(frame)

        # Compute floor and wall homographies
        self.homographies["floor"] = self._compute_floor_homography(keypoints_per_class)
        self.homographies["wall"] = self._compute_wall_homography(keypoints_per_class)

        return self.homographies, keypoints_per_class

    def detect_wall_color(self, frame, keypoints_per_class=None):
        """
        Detect if the front wall is white (indicating black ball usage).

        Args:
            frame: Input frame
            keypoints_per_class: Optional dict of detected keypoints. If None, will detect.

        Returns:
            dict with:
                - 'is_white': bool indicating if wall is white
                - 'mean_brightness': float (0-255) mean brightness of wall
                - 'mean_saturation': float (0-255) mean saturation of wall
                - 'wall_color_rgb': tuple of mean (R, G, B) values
                - 'recommended_ball': 'black' or 'yellow' based on wall color
        """
        # Get keypoints if not provided
        if keypoints_per_class is None:
            keypoints_per_class = self.detect_keypoints(frame)

        if "front-wall-down" not in keypoints_per_class:
            raise ValueError("Front wall not detected - cannot determine wall color")

        # Get front wall polygon points
        wall_points = keypoints_per_class["front-wall-down"]
        ...
        return {
            ...
            "recommended_ball": recommended_ball,
        }
